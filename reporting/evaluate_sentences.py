...
def generate_sentence_report(raw_data):
    ...
    for run_id in df["run_id"].unique():
        df_run = df[df["run_id"] == run_id]
        file_name =f"matching_sentences_details_{run_id}.csv"
    ...
    for run_id, summary_df in summary_dict.items():
        file_name = f"matching_sentences_summary_{run_id}.csv"
...
if __name__ == "__main__":
    ...
    generate_sentence_report(raw_data)
